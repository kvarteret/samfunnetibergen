...
export function RecurrenceBuilder({ onChange }: RecurrenceBuilderProps) {
    ...
    const recurrence = useMemo(
        () => buildRecurrence({ frequency, interval, weekdays, endType, count, untilDate }),
    ...
    return (
        <div className="space-y-5 border-2 border-border bg-secondary/10 p-6">
            <RecurrenceHeader />
            <RecurrenceFrequencyField frequency={frequency} onFrequencyChange={setFrequency} />
...
function RecurrenceFrequencyField({ frequency, onFrequencyChange }: RecurrenceFrequencyFieldProps) {
    return (
        ...
        <fieldset className="space-y-3">
            ...
            <div className="space-y-2">
                {endTypeOptions.map(option => (
                    <label className="flex cursor-pointer items-center gap-3" key={option.value}>
                        <input
...
function RecurrencePreview({ preview }: RecurrencePreviewProps) {
    ...
    return (
        <div className="border-l-4 border-primary py-1 pl-4">
            ...
            <p className="text-sm font-heading capitalize text-foreground">{preview}</p>
        </div>
    )
}
