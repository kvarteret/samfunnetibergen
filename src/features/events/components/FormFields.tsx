...
import type { SelectOption } from "../domain/formState"
...
export function SectionHeader({ number, title }: SectionHeaderProps) {
    return (
        <div className="flex items-baseline gap-4 border-b-2 border-border pb-4">
            <span className="font-heading text-4xl leading-none text-primary">{number}</span>
...
export function CheckboxSquare({ checked, onChange }: CheckboxSquareProps) {
    return (
        <span className="relative mt-0.5">
            ...
            >
                {checked && <Check aria-hidden className="size-3 text-primary-foreground" />}
            </span>
        </span>
    )
}
