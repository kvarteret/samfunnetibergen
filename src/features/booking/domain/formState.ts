import type { RoomBookingPayload } from "@/app/actions/submit-room-booking"
...
export type BookingAction = { type: "SET"; key: keyof BookingFormState; value: BookingFieldValue }

export type SetBookingField = <Key extends keyof BookingFormState>(
    key: Key,
) => (value: BookingFormState[Key]) => void

export function reducer(state: BookingFormState, action: BookingAction): BookingFormState {
    switch (action.type) {
        case "SET":
...
export const isExternalBooker = (bookerType: BookerType): boolean => bookerType !== "intern"
...
export function buildBookingPayload(
    ...
    return {
        ...
        studentOrgName: state.bookerType === "studentorg" ? state.studentOrgName : undefined,
        invoiceAddress: isExternal ? state.invoiceAddress : undefined,
        orgNumber: isExternal && state.orgNumber.trim() ? Number(state.orgNumber) : undefined,
    }
}
