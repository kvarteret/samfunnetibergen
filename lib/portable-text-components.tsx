...
export function PortableTextContent({ value }: PortableTextContentProps) {
    ...
    return value.map((block, index, blocks) => {
        if (block._type === "block") {
            ...
            return (
                <div className="prose prose-neutral max-w-none dark:prose-invert" key={block._key}>
                    <PortableText components={portableTextComponents} value={groupedTextBlocks} />
                </div>
            )
        }

        return <PortableText components={portableTextComponents} key={block._key} value={block} />
    })
}
...
function PortableTextImage({ value }: { value: PortableTextImageValue }) {
    ...
    return (
        <figure className="my-10">
            ...
            {value.caption && (
                <figcaption className="mt-2 text-sm text-foreground/70">{value.caption}</figcaption>
            )}
        </figure>
    )
}
