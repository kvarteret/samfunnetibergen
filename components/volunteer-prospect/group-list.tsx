...
type VolunteerProspectGroupListProps = {
    ...
    onChoosePreference: (groupSlug: VolunteerGroupSlug) => void
...
export function VolunteerProspectGroupList({
    ...
    return (
        <div className="grid gap-5">
            {groups.map(group => (
                <VolunteerProspectGroupCard
                    accordionActionLabel={accordionActionLabel}
                    detailActionLabel={detailActionLabel(group.name ?? "")}
                    group={group}
                    isSecondChoice={secondChoiceGroupSlug === group.slug}
                    isSelected={firstChoiceGroupSlug === group.slug}
                    key={group.slug}
                    onChoosePreference={onChoosePreference}
                    selectionActionIdleLabel={selectionActionIdleLabel}
                    selectionActionPrimaryLabel={selectionActionPrimaryLabel}
                    selectionActionSecondaryLabel={selectionActionSecondaryLabel}
                />
            ))}
        </div>
    )
...
interface VolunteerProspectGroupCardProps {
    accordionActionLabel: string
    detailActionLabel: string
    group: VolunteerGroupContent
...
function VolunteerProspectGroupCard({
    accordionActionLabel,
    detailActionLabel,
    group,
    isSecondChoice,
    ...
    selectionActionPrimaryLabel,
    selectionActionSecondaryLabel,
}: VolunteerProspectGroupCardProps) {
    const selectionActionLabel = isSelected
        ? selectionActionPrimaryLabel
        : isSecondChoice
          ? selectionActionSecondaryLabel
          : selectionActionIdleLabel
        ...
        >
            <div className="grid gap-5 p-6">
                    ...
                    <div className="space-y-2">
                        <p
                            className={cn(
                                ...
                                isSelected ? "text-primary-foreground/75" : "text-foreground/65",
                            )}
                        >
                            {group.eyebrow}
                        </p>
                            ...
                            {group.accordionSections.map(section => (
                                <div className="grid gap-2" key={section.title}>
                                    ...
                                    {section.paragraphs.map(paragraph => (
                                        <p className="text-sm leading-6" key={paragraph}>
                                            {paragraph}
                                        </p>
                                    ))}
                                </div>
                ...
                <div className="flex flex-wrap justify-between gap-3">
                    <Button asChild className="w-full sm:w-auto" type="button" variant="neutral">
                        <Link href={`/grupper/${group.slug}`}>{detailActionLabel}</Link>
                    </Button>
                </div>
            </div>
        </article>
