...
- [ ] Milestone 6 — Frontend surfaces (feed already done in M4): remove read-time rrule expansion from public reads (`computeAllDates` fallback in `src/features/events/domain/dates.ts`); recurring badge follows `eventKind`; series/festival parent detail pages list their children via `fetchEventChildren`; cancelled/postponed badge + detail states.
