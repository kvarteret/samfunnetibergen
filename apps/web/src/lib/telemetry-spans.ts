...
export function sanitizeSpan(span: ReadableSpan): ReadableSpan {
  ...
  for (const [key, value] of Object.entries(span.attributes)) {
    if (
      /header|cookie|authorization|password|token|body|statement|user_agent|referer|exception\.(message|stacktrace)/i.test(
        key,
      )
    )
      continue
...
export function withSanitizedSpans(exporter: SpanExporter): SpanExporter {
  return {
    export: (spans, callback) =>
      exporter.export(spans.map(sanitizeSpan), callback),
