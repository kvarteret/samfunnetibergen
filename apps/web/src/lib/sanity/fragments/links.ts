...
export const localizedSourceLinkProjection = (localeParam = "$locale") => `{
    _key,
    "label": coalesce(localizedLabel[language == ${localeParam}][0].value, localizedLabel[language == "nb"][0].value, label, "Lenke uten navn"),
    "kind": select(
        linkType == "internalPage" => "internalDocument",
        linkType == "internalPath" => "internalPath",
        linkType == "external" => "external"
