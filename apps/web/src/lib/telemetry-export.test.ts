...
vi.mock("@vercel/functions", () => ({ waitUntil: (promise: Promise<void>) => retained.push(promise) }))
  ...
  it("keeps the invocation alive until HTTP completion, including failures", async () => {
    for (const code of [0, 1]) {
      ...
      const exporter = withExportLifetime({
        export(_items: string[], next: (result: {code: number}) => void) { done = next },
        shutdown: async () => {},
      })
      exporter.export(["record"], callback)
      let resolved = false
      const pending = retained.at(-1)!.then(() => { resolved = true })
