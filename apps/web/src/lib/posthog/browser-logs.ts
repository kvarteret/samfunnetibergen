...
export function normalizeBrowserLog(
  ...
  const sanityState = body.includes("<SanityLive>")
    ? body.includes("attempting to reconnect")
      ? "reconnecting"
      : body.includes("connected and listening")
        ? "connected"
        : undefined
    : undefined
  return {
    ...
    attributes: {
      ...
      ...(sanityState
        ? { event: "sanity.live.connection", "connection.state": sanityState }
        : {}),
    },
  }
}
